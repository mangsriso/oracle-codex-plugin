...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run /ox:status to list known jobs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run /ox:status to inspect known jobs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check /ox:status and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run /ox:status to inspect active jobs.`);
  }
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) {
    throw new Error("Multiple Codex jobs are active. Pass a job id to /ox:cancel.");
  }
