...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`/ox:status ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`/ox:cancel ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: /ox:cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: /ox:result ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: /ox:review --wait");
    lines.push("  Stricter review: /ox:adversarial-review --wait");
  }
...
export function renderCancelReport(job) {
  ...
  lines.push("- Check `/ox:status` for the updated queue.");
