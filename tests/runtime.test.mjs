...
import { loadBrokerSession, saveBrokerSession } from "../plugins/ox/scripts/lib/broker-lifecycle.mjs";
...
test("transfer reuses the ledgered thread when the same source is imported twice", () => {
  const home = makeTempDir();
  const repo = path.join(home, "repo");
  const binDir = makeTempDir();
  const projectDir = path.join(home, ".claude", "projects", "-repo");
  ...
  fs.writeFileSync(
    sourcePath,
    `${JSON.stringify({ type: "user", cwd: repo, message: { role: "user", content: "Import me once." } })}\n`,
    "utf8"
  );
  ...
  const first = run("node", [SCRIPT, "transfer", "--source", sourcePath, "--json"], { cwd: repo, env });
  assert.equal(first.status, 0, first.stderr);
  const firstPayload = JSON.parse(first.stdout);
  assert.equal(firstPayload.threadId, "thr_1");

  const second = run("node", [SCRIPT, "transfer", "--source", sourcePath, "--json"], { cwd: repo, env });
  ...
  const ledger = JSON.parse(
    fs.readFileSync(path.join(home, ".codex", "external_agent_session_imports.json"), "utf8")
  );
