...
function makeVersionFixture() {
  ...
  writeJson(path.join(root, "plugins", "ox", ".claude-plugin", "plugin.json"), {
    name: "ox",
    version: "1.0.2"
  });
...
test("bump-version updates every release manifest", () => {
  ...
  assert.equal(readJson(path.join(root, "plugins", "ox", ".claude-plugin", "plugin.json")).version, "1.2.3");
...
test("bump-version check mode reports stale metadata", () => {
  ...
  assert.match(result.stderr, /plugins\/ox\/\.claude-plugin\/plugin\.json version/);
